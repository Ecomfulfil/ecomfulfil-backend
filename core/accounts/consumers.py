...
ACCOUNT_GROUP_TEMPLATE = (
    "account_{}"  # only the account owner is subscribed to this group
)
ONLINE_STATUS_TRACKING_GROUP_TEMPLATE = (
    "online_{}"  # peers subscribe to this channel for online status notifications
)
...
class AccountConsumer(JsonWebsocketConsumer):
    ...
    def track_online_status(self, event):
        logger.debug(
            "Got event in channel %s (account_number: %s): %s",
            self.channel_name,
            self.account_number,
            event,
        )
